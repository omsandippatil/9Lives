...
export default function JavaTheoryPage() {
  ...
  const [currentTopic, setCurrentTopic] = useState<JavaTheory | null>(null)
  ...
  const loadJavaTheoryData = async () => {
    try {
      ...
      console.log('Fetching Java theory data for user ID:', userId)

      // Fetch user's Java progress
      const { data: userProfile, error: profileError } = await supabase
        .from('users')
        .select('id, java_lang_covered')
        .eq('id', userId)
        .single()
      ...
      const nextTopicId = userProfile.java_lang_covered + 1
      setNextTopicNumber(nextTopicId)

      console.log('Current Java topics covered:', userProfile.java_lang_covered)
      console.log('Next topic ID to fetch:', nextTopicId)

      // Fetch the next Java theory topic
      const { data: javaTheory, error: theoryError } = await supabase
      ...
      if (theoryError) {
        console.error('Failed to fetch Java theory:', theoryError)
        if (theoryError?.code === 'PGRST116') {
          setError('No more Java topics available')
        } else {
          setError('Failed to load Java theory: ' + (theoryError?.message || 'Unknown error'))
        }
        setLoading(false)
        return
      }
      ...
      if (!javaTheory) {
        setError('Java theory topic not found')
        setLoading(false)
        return
      }

      console.log('Java theory loaded successfully:', javaTheory)
      setCurrentTopic(javaTheory)
      setLoading(false)

    } catch (err) {
      console.error('Java theory load error:', err)
      setError('Failed to load Java theory: ' + (err instanceof Error ? err.message : 'Unknown error'))
      setLoading(false)
    }
  }
  ...
  const handleTopicClick = () => {
    if (currentTopic) {
      router.push(`/languages/java/${currentTopic.id}`)
    }
  }
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="font-mono text-gray-600">Loading your Java journey...</p>
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="font-mono text-gray-600 mb-6">No Java theory data available.</p>
          <button 
            onClick={loadJavaTheoryData}
      ...
      <header className="border-b border-gray-100 py-6">
        <div className="max-w-7xl mx-auto flex items-center justify-between px-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-light">Java Theory</h1>
          </div>
          
          <div className="flex items-center gap-4">
            <div className="text-center">
              <p className="text-xs text-gray-400 uppercase tracking-wider">Progress</p>
              <p className="text-lg font-light">{profile.java_lang_covered}/100</p>
            </div>
          </div>
        </div>
      </header>
          ...
          <div className="p-8">
            ...
            <div className="mb-6">
              <div className="flex justify-between items-center mb-3">
                ...
                <span className="font-mono text-sm text-gray-600">{profile.java_lang_covered}/100</span>
              </div>
              <div className="w-full bg-gray-50 h-3 overflow-hidden">
                <div 
                  className="bg-black h-full transition-all duration-700 ease-out"
                  style={{ width: `${Math.min((profile.java_lang_covered / 100) * 100, 100)}%` }}
                />
              </div>
            </div>
            ...
            <div className="text-center mb-6">
              <p className="font-mono text-base text-gray-600 mb-3">
                🐾 {profile.java_lang_covered} topics pounced • {100 - profile.java_lang_covered} to go
              </p>
