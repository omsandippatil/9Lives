...
export default function SqlTheoryPage() {
  const [profile, setProfile] = useState<UserProfile | null>(null)
  const [currentTopic, setCurrentTopic] = useState<SqlTheory | null>(null)
  ...
  const loadSqlTheoryData = async () => {
    try {
      ...
      console.log('Fetching SQL theory data for user ID:', userId)

      // Fetch user's SQL progress
      const { data: userProfile, error: profileError } = await supabase
        .from('users')
        .select('id, sql_lang_covered')
        .eq('id', userId)
        .single()
      ...
      const nextTopicId = userProfile.sql_lang_covered + 1
      setNextTopicNumber(nextTopicId)

      console.log('Current SQL topics covered:', userProfile.sql_lang_covered)
      console.log('Next topic ID to fetch:', nextTopicId)

      // Fetch the next SQL theory topic
      const { data: sqlTheory, error: theoryError } = await supabase
      ...
      if (theoryError) {
        console.error('Failed to fetch SQL theory:', theoryError)
        if (theoryError?.code === 'PGRST116') {
          setError('No more SQL topics available')
        } else {
          setError('Failed to load SQL theory: ' + (theoryError?.message || 'Unknown error'))
        }
        setLoading(false)
        return
      }
      ...
      if (!sqlTheory) {
        setError('SQL theory topic not found')
        setLoading(false)
        return
      }

      console.log('SQL theory loaded successfully:', sqlTheory)
      setCurrentTopic(sqlTheory)
      setLoading(false)

    } catch (err) {
      console.error('SQL theory load error:', err)
      setError('Failed to load SQL theory: ' + (err instanceof Error ? err.message : 'Unknown error'))
      setLoading(false)
    }
  }
  ...
  const handleTopicClick = () => {
    if (currentTopic) {
      router.push(`/languages/sql/${currentTopic.id}`)
    }
  }
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="font-mono text-gray-600">Loading your SQL journey...</p>
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="font-mono text-gray-600 mb-6">No SQL theory data available.</p>
          <button 
            onClick={loadSqlTheoryData}
      ...
      <header className="border-b border-gray-100 py-6">
        <div className="max-w-7xl mx-auto flex items-center justify-between px-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-light">SQL Theory</h1>
          </div>
          
          <div className="flex items-center gap-4">
            <div className="text-center">
              <p className="text-xs text-gray-400 uppercase tracking-wider">Progress</p>
              <p className="text-lg font-light">{profile.sql_lang_covered}/100</p>
            </div>
          </div>
        </div>
      </header>
