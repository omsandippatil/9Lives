...
export default function FundamentalTopicQuestionsPage() {
  ...
  const loadTopicData = async () => {
    try {
      ...
      console.log(`Fetching fundamental questions ${startId} to ${endId}`)
      ...
      console.log(`Loaded ${questionsData?.length || 0} fundamental questions`)
      setQuestions(questionsData || [])
      setLoading(false)

    } catch (err) {
      console.error('Fundamental topic data load error:', err)
          ...
          <div className="text-center py-16 border-t border-gray-100">
            <div className="max-w-2xl mx-auto">
              ...
              <div className="text-sm text-gray-400 font-mono">
                {questions.filter(q => isQuestionCompleted(q.id)).length} of {questions.length} fundamental questions completed
              </div>
            </div>
          </div>
        </div>
