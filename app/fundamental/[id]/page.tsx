...
interface FundamentalQuestionData {
  id: number
  question: string
  answer: string
...
export default function FundamentalQuestionsPage() {
  ...
  const [questionData, setQuestionData] = useState<FundamentalQuestionData | null>(null)
  ...
  const [fundamentalQuestions, setFundamentalQuestions] = useState(0)
  const [showFishAnimation, setShowFishAnimation] = useState(false)
  
  const [timeLeft, setTimeLeft] = useState(60) // Time for fundamental questions
  ...
  useEffect(() => {
    if (timeLeft > 0) {
      ...
      if (!hasAttempted) {
        ...
        fetch('/api/update/today?inc=fundamental_questions_attempted', {
          method: 'POST',
          credentials: 'include'
        }).catch(console.error)
  ...
  const fetchUserData = async () => {
    try {
      ...
      const { data: userData, error: fetchError } = await supabase
        .from('users')
        .select('current_streak, total_points, fundamental_questions_attempted')
        .eq('id', userId)
        .single()
      ...
      setFundamentalQuestions(userData?.fundamental_questions_attempted || 0)
    } catch (error) {
  ...
  const updateUserProgress = async () => {
    try {
      ...
      if (currentQuestionId === fundamentalQuestions + 1) {
        let userId = getCookie('client-user-id') || 
                    (typeof localStorage !== 'undefined' ? localStorage.getItem('client-user-id') : null) || 
                    (typeof localStorage !== 'undefined' ? localStorage.getItem('supabase-user-id') : null)
        ...
        if (userId) {
          const { error } = await supabase
            .from('users')
            .update({ 
              fundamental_questions_attempted: currentQuestionId,
              total_points: totalPoints + 1 // Fundamental questions give 1 point
            })
            .eq('id', userId)

          if (!error) {
            setFundamentalQuestions(currentQuestionId)
  ...
  const handleNext = () => {
    ...
    router.push(`/fundamental/${nextId}`)
  }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 font-mono text-lg mb-6">Loading fundamental concept...</p>
          <LoadingProgressBar />
        </div>
      </div>
    )
          ...
          <div className="max-w-7xl mx-auto flex justify-between items-center px-4">
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="text-center">
                ...
                <p className="text-sm font-light text-blue-600">Fundamental</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        <div className="text-center mb-12 bg-white shadow-sm border p-8">
          ...
          <p className="text-base text-gray-400 font-light">
            Master fundamental concepts, build strong foundations 🧠
          </p>
        ...
        <div className="text-center py-8 border-t border-gray-200 bg-white shadow-sm">
          ...
          <p className="text-lg text-gray-600 font-light mb-6">
            {canProceed ? 'Ready to explore the next fundamental concept?' : 'Keep reading to unlock the next concept!'}
          </p>
          ...
          {canProceed && (
            <p className="text-sm text-blue-600 mt-2 font-light">
              🎉 Excellent! You've earned 1 point for completing this fundamental concept.
            </p>
          )}
        </div>
      </main>
