// app/api/update/coding-questions/route.ts
...
export async function POST(request: NextRequest) {
  try {
    console.log('=== UPDATE CODING QUESTIONS API START ===')
    ...
    if (userId) {
      ...
      const { data, error } = await supabaseAdmin
        .from('users')
        .select('id, email, coding_questions_attempted')
        .eq('id', userId)
        .single()
    ...
    // Calculate new count
    const currentCount = currentProfile.coding_questions_attempted || 0
    const newCount = currentCount + 1

    console.log('Updating coding questions:', {
      userId: finalUserId,
      currentCount,
      newCount
    })

    // Update the coding questions count
    const { data: updatedProfile, error: updateError } = await supabaseAdmin
      .from('users')
      .update({ 
        coding_questions_attempted: newCount,
    ...
    if (updateError) {
      ...
      return NextResponse.json(
        { 
          error: 'Failed to update coding questions count',
    ...
    console.log('=== UPDATE CODING QUESTIONS API END ===')

    return NextResponse.json({ 
      success: true,
      message: `Successfully updated coding questions from ${currentCount} to ${newCount}`,
      data: {
        userId: updatedProfile.id,
        email: updatedProfile.email,
        previousCount: currentCount,
        newCount: updatedProfile.coding_questions_attempted,
        incrementedBy: 1
      }
    })

  } catch (error) {
    console.error('Unexpected error in coding questions update:', error)
    return NextResponse.json(
      { 
