...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get current question number from questions_done table (java_lang_theory column)
    const { data: questionDoneData, error: questionDoneError } = await supabase
      .from('questions_done')
      .select('java_lang_theory')
      .eq('id', 1)
      .single();
    ...
    const nextQuestionId = questionDoneData.java_lang_theory + 1;

    // Fetch the next concept from java_lang_theory table
    ...
    const groqPrompt = `Generate a comprehensive, educational explanation for the Java programming concept. Create structured content that's easy to understand with real-world analogies. Respond with valid JSON only.
...
- Format: $#\\ncode here\\n #$
- Never use $# java or $#java - just $# then newline
...
- Include practical examples and analogies`;
    ...
    const groqResponse = await fetch(GROQ_API_URL, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert Java programming instructor and technical writer. Create comprehensive, educational content with clear explanations, practical examples, structured tables, real-world analogies, and proper formatting. CRITICAL: Always use "$#" to start code blocks and " #$" (with one space before) to end code blocks. Never add language specifiers after $#. Always respond with valid JSON format. Use tables to organize information when it makes the content clearer. Include step-by-step explanations and relatable analogies to make complex concepts easy to understand.'
          },
          {
            role: 'user',
            content: groqPrompt
    ...
    // Update the java_lang_theory table with the processed theory
    ...
    const { error: incrementError } = await supabase
      .from('questions_done')
      .update({
        java_lang_theory: nextQuestionId
      })
      .eq('id', 1);
    ...
    return NextResponse.json({
      success: true,
      message: 'Enhanced Java theory generated and saved successfully',
      data: {
...
export async function POST(request: NextRequest) {
  return NextResponse.json({ 
    error: 'Method not allowed. Use GET to generate Java theory content.' 
  }, { status: 405 });
}