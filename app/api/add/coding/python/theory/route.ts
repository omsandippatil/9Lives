...
const groqApiKey = process.env.PYTHON_THEORY_GROQ_API_KEY!;
...
async function getNextQuestionNumber(): Promise<number> {
  try {
    ...
    if (existingRecord && existingRecord.coding_theory_python) {
      // If coding_theory_python is a number, increment it by 1
      if (typeof existingRecord.coding_theory_python === 'number') {
        nextQuestionNumber = existingRecord.coding_theory_python + 1;
      } 
      // If it's an array (legacy), get the max and increment
      else if (Array.isArray(existingRecord.coding_theory_python) && existingRecord.coding_theory_python.length > 0) {
        const maxQuestionNumber = Math.max(...existingRecord.coding_theory_python);
...
// Simplified Groq API prompt template for Python
const createGroqPrompt = (question: string, approach: string): string => {
  return `Analyze this Python coding question and approach. Return ONLY a valid JSON object with no additional text.
...
- syntax_explanation should include Python methods, concepts, or syntax used in this solution
- Each syntax item should be: "syntax_name": "simple explanation of what it does"
- Focus on actual Python syntax/methods used in the solution
  ...
  const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a Python programming expert. Always respond with valid JSON only. Provide simple, clear explanations.'
        },
        {
          role: 'user',
          content: prompt
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (existingAnalysis) {
      ...
      // Update questions_done table for coding_theory_python
      try {
        await updateQuestionsDoneTable(questionNumber, 'coding_theory_python');
      ...
      // Step 6: Store analysis in python_coding_theory table
      ...
      // Step 7: Update questions_done table for coding_theory_python
      try {
        await updateQuestionsDoneTable(questionNumber, 'coding_theory_python');
