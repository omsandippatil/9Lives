...
export default function HRQuestionsPage() {
  ...
  const [questionData, setQuestionData] = useState<HRQuestionData | null>(null)
  ...
  const [hrQuestions, setHrQuestions] = useState(0)
  const [showFishAnimation, setShowFishAnimation] = useState(false)
  
  const [timeLeft, setTimeLeft] = useState(60) // Time for HR questions
  ...
  useEffect(() => {
    if (timeLeft > 0) {
      ...
      if (!hasAttempted) {
        ...
        fetch('/api/update/today?inc=hr_questions_attempted', {
          method: 'POST',
          credentials: 'include'
        }).catch(console.error)
  ...
  const fetchUserData = async () => {
    try {
      ...
      const { data: userData, error: fetchError } = await supabase
        .from('users')
        .select('current_streak, total_points, hr_questions_attempted')
        .eq('id', userId)
        .single()
      ...
      setHrQuestions(userData?.hr_questions_attempted || 0)
    } catch (error) {
  ...
  const updateUserProgress = async () => {
    try {
      ...
      if (currentQuestionId === hrQuestions + 1) {
        let userId = getCookie('client-user-id') || 
                    (typeof localStorage !== 'undefined' ? localStorage.getItem('client-user-id') : null) || 
                    (typeof localStorage !== 'undefined' ? localStorage.getItem('supabase-user-id') : null)
        ...
        if (userId) {
          const { error } = await supabase
            .from('users')
            .update({ 
              hr_questions_attempted: currentQuestionId,
              total_points: totalPoints + 1 // HR questions give 1 point
            })
            .eq('id', userId)

          if (!error) {
            setHrQuestions(currentQuestionId)
  ...
  useEffect(() => {
    const fetchQuestionData = async () => {
      try {
        ...
        if (data.response) {
          const sections = parseAnswerContent(data.response)
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 font-mono text-lg mb-6">Loading HR question...</p>
          <LoadingProgressBar />
        </div>
      </div>
    )
          ...
          <div className="max-w-7xl mx-auto flex justify-between items-center px-4">
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="text-center">
                ...
                <p className="text-sm font-light text-blue-600">HR</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        <div className="text-center mb-12 bg-white shadow-sm border p-8">
          ...
          <div className="bg-blue-50 border-l-4 border-blue-400 p-6 mb-6">
            <h3 className="text-2xl font-medium text-blue-900 mb-2">Question:</h3>
            <p className="text-xl text-blue-800 font-bold leading-relaxed">
              {questionData.question}
            </p>
          </div>
          <p className="text-base text-gray-400 font-light">
            Master HR concepts, build professional skills 💼
          </p>
        ...
        {parsedSections.length > 0 ? (
          ...
          <div className="bg-white shadow-sm border p-8 mb-12">
            <ReactMarkdown 
              ...
              remarkPlugins={[remarkGfm]}
            >
              {processAnswerContent(questionData.response)}
            </ReactMarkdown>
          </div>
        )}
        ...
        <div className="text-center py-8 border-t border-gray-200 bg-white shadow-sm">
          ...
          <p className="text-lg text-gray-600 font-light mb-6">
            {canProceed ? 'Ready to explore the next HR question?' : 'Keep reading to unlock the next question!'}
          </p>
          ...
          {canProceed && (
            <p className="text-sm text-blue-600 mt-2 font-light">
              🎉 Excellent! You've earned 1 point for completing this HR question.
            </p>
          )}
        </div>
      </main>
