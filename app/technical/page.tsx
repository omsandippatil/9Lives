...
export default function TechnicalQuestionsPage() {
  ...
  const isTopicCompleted = (topicId: number): boolean => {
    ...
    const completedTopics = Math.floor(profile.technical_questions_attempted / QUESTIONS_PER_TOPIC)
  ...
  useEffect(() => {
    loadTechnicalQuestionData()
  }, [])

  const loadTechnicalQuestionData = async () => {
    try {
      ...
      console.log('Fetching technical questions data for user ID:', userId)

      // Fetch user's technical questions progress
      ...
      const nextQuestionId = userProfile.technical_questions_attempted + 1
      ...
      console.log('Current technical questions attempted:', userProfile.technical_questions_attempted)
      console.log('Next question ID to fetch:', nextQuestionId)

      // Fetch the next technical question
      const { data: technicalQuestion, error: questionError } = await supabase
      ...
      if (questionError) {
        console.error('Failed to fetch technical question:', questionError)
        if (questionError?.code === 'PGRST116') {
          setError('No more technical questions available')
        } else {
          setError('Failed to load technical question: ' + (questionError?.message || 'Unknown error'))
        }
        setLoading(false)
        return
      }
      ...
      if (!technicalQuestion) {
        setError('Technical question not found')
        setLoading(false)
        return
      }

      console.log('Technical question loaded successfully:', technicalQuestion)
      setCurrentQuestion(technicalQuestion)
      setLoading(false)

    } catch (err) {
      console.error('Technical question load error:', err)
      setError('Failed to load technical question: ' + (err instanceof Error ? err.message : 'Unknown error'))
      setLoading(false)
    }
  }
  ...
  const handleQuestionClick = () => {
    if (currentQuestion) {
      router.push(`/technical/${currentQuestion.id}`)
    }
  }

  const handleTopicClick = (topicId: number) => {
    if (isTopicAccessible(topicId)) {
      router.push(`/technical/list/${topicId}`)
    }
  }
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="font-mono text-gray-600">Loading your technical challenge...</p>
      ...
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <div className="space-y-4">
            ...
            <button 
              onClick={loadTechnicalQuestionData}
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="font-mono text-gray-600 mb-6">No technical questions available.</p>
          <button 
            onClick={loadTechnicalQuestionData}
      ...
      <header className="border-b border-gray-100 py-6">
        <div className="max-w-7xl mx-auto flex items-center justify-between px-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-light">Technical Questions</h1>
          </div>
          ...
          <div className="text-center py-16 border-t border-gray-100 mt-12">
            <div className="max-w-2xl mx-auto">
              ...
              <p className="font-mono text-gray-500 mb-4">
                Keep pouncing on those technical challenges!
              </p>
              <div className="text-sm text-gray-400 font-mono">
                Progress: {profile?.technical_questions_attempted || 0} / {TOTAL_QUESTIONS} questions completed
              </div>
            </div>
          </div>
        </div>
